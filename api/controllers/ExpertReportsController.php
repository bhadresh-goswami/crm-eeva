...
class ExpertReportsController {
    public function daily($authUser): void {
        try {
            ...
            $stmt = $conn->prepare("SELECT t.id, t.title, t.description, DATE(t.due_date) task_date, COALESCE(c.name,'') candidate_name, COALESCE(tt.name,'Unspecified') task_type, COALESCE(ts.name,'Pending') status_name, t.start_time, t.end_time, t.task_start_time, t.task_end_time, COALESCE(t.duration,0) duration, tf.id feedback_id, COALESCE(tf.area_of_improvements,'') feedback, COALESCE(tf.overall,0) feedback_score FROM tasks t INNER JOIN task_assignments ta ON ta.task_id=t.id AND ta.user_id=? LEFT JOIN candidates c ON c.id=t.candidate_id LEFT JOIN task_types tt ON tt.id=t.task_type_id LEFT JOIN task_status_master ts ON ts.id=t.status_id LEFT JOIN task_feedback tf ON tf.task_id=t.id WHERE DATE(t.due_date)=? GROUP BY t.id ORDER BY t.start_time,t.id");
            $stmt->execute([$userId, $date]); $tasks = $stmt->fetchAll(PDO::FETCH_ASSOC);
            $summary = ['assigned'=>count($tasks),'completed'=>0,'pending'=>0,'cancelled'=>0,'scheduled_minutes'=>0,'actual_minutes'=>0,'pending_feedback'=>0]; $types=[]; $positive=[]; $negative=[]; $eligible=0; $successful=0;
            ...
            $feedbackEligibleSql = FeedbackEligibility::sql('tt.name', 'ts.name');
            $pendingStmt = $conn->prepare("SELECT t.id task_id, DATE(t.due_date) task_date, COALESCE(c.name,'') candidate_name, COALESCE(tt.name,'Unspecified') task_type, t.task_end_time completed_at FROM tasks t INNER JOIN task_assignments ta ON ta.task_id=t.id AND ta.user_id=? LEFT JOIN candidates c ON c.id=t.candidate_id LEFT JOIN task_types tt ON tt.id=t.task_type_id LEFT JOIN task_status_master ts ON ts.id=t.status_id LEFT JOIN task_feedback tf ON tf.task_id=t.id WHERE DATE(t.due_date)<=? AND tf.id IS NULL AND ({$feedbackEligibleSql}) GROUP BY t.id ORDER BY t.due_date DESC,t.id DESC");
            $pendingStmt->execute([$userId,$date]); $pendingFeedback=$pendingStmt->fetchAll(PDO::FETCH_ASSOC);
            $pendingFeedbackCounts=[]; foreach($pendingFeedback as $item){$pendingFeedbackCounts[$item['task_type']]=($pendingFeedbackCounts[$item['task_type']]??0)+1;} ksort($pendingFeedbackCounts);
            $summary['pending_feedback']=count($pendingFeedback);
            ...
            echo json_encode(['success'=>true,'data'=>['report_date'=>$date,'expert'=>$expert,'summary'=>$summary,'task_types'=>$types,'tasks'=>$tasks,'break_minutes'=>(int)$breakStmt->fetchColumn(),'success_ratio'=>['eligible'=>$eligible,'successful'=>$successful,'other'=>$eligible-$successful,'percentage'=>$eligible?round($successful/$eligible*100):null],'pending_feedback'=>$pendingFeedback,'pending_feedback_counts'=>$pendingFeedbackCounts,'positive_feedback'=>array_slice($positive,0,3),'negative_feedback'=>array_slice($negative,0,3),'delivery'=>$sent]]);
