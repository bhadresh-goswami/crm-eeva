...
class ExpertReportsController {
    public function daily($authUser): void {
        try {
            ...
            $summary = ['assigned'=>count($tasks),'completed'=>0,'pending'=>0,'cancelled'=>0,'scheduled_minutes'=>0,'actual_minutes'=>0,'pending_feedback'=>0]; $types=[]; $positive=[]; $negative=[]; $pendingFeedback=[]; $eligible=0; $successful=0;
            foreach ($tasks as &$task) {
                ...
                if ($task['feedback_id'] === null && FeedbackEligibility::isEligible($type, (string)$task['status_name'])) {
                    $summary['pending_feedback']++;
                    $pendingFeedback[]=['task_id'=>(int)$task['id'],'task_type'=>$type,'candidate_name'=>$task['candidate_name'],'completed_at'=>$task['task_end_time']];
                }
            ...
            echo json_encode(['success'=>true,'data'=>['report_date'=>$date,'expert'=>$expert,'summary'=>$summary,'task_types'=>$types,'tasks'=>$tasks,'break_minutes'=>(int)$breakStmt->fetchColumn(),'success_ratio'=>['eligible'=>$eligible,'successful'=>$successful,'other'=>$eligible-$successful,'percentage'=>$eligible?round($successful/$eligible*100):null],'pending_feedback'=>$pendingFeedback,'positive_feedback'=>array_slice($positive,0,3),'negative_feedback'=>array_slice($negative,0,3),'delivery'=>$sent]]);
