...
const ManagerReportPageBase = ({ title, subtitle, columns, endpoint }: ReportPageProps) => {
  ...
  return (
    <div className="page-container">
      ...
      }}>Export Excel</button></div><div className="table-wrapper manager-reports-table__wrapper"><table className="table table-hover table-bordered align-middle manager-reports-table mb-0"><thead><tr>{columns.map((column) => <th key={column.key}>{column.key === 'action' ? <span>{column.label}</span> : <button type="button" className="manager-sort" onClick={() => onSort(column.key)}><span>{column.label}</span><BsArrowDownUp size={12} /></button>}</th>)}</tr></thead><tbody>{loading ? <tr>{columns.map((_, i) => <td key={i}><div className="placeholder-glow"><span className="placeholder col-10" /></div></td>)}</tr> : sortedRows.length === 0 ? <tr><td colSpan={columns.length} className="text-center">No data found.</td></tr> : sortedRows.map((row, idx) => <tr key={`${String(mapValue(row, 'taskId'))}-${idx}`}>{columns.map((column) => column.key === 'action' ? <td key={`${idx}-action`} className="text-center"><button className="btn btn-outline-primary btn-sm rounded-pill" title="View Details" onClick={() => void openDetails(Number(row.task_id ?? 0))}><BsEye size={15} className="text-primary" /></button></td> : <td key={`${idx}-${column.key}`} className="manager-cell" title={String(mapValue(row, column.key))}><span className="manager-cell-ellipsis">{String(mapValue(row, column.key))}</span></td>)}</tr>)}</tbody></table></div></div>

      <div className="card py-2 px-3 d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-2"><small className="text-muted">Page {filters.page ?? 1}</small><nav><ul className="pagination mb-0"><li className={`page-item ${(filters.page ?? 1) <= 1 ? 'disabled' : ''}`}><button className="page-link" onClick={() => { const page = Math.max(1, (filters.page ?? 1) - 1); setFilters((p) => ({ ...p, page })); void load({ page }) }}>Previous</button></li>{[Math.max(1, (filters.page ?? 1) - 1), filters.page ?? 1, (filters.page ?? 1) + 1].filter((v, i, a) => a.indexOf(v) === i).map((pageNo) => <li key={pageNo} className={`page-item ${pageNo === (filters.page ?? 1) ? 'active' : ''}`}><button className="page-link" onClick={() => { setFilters((p) => ({ ...p, page: pageNo })); void load({ page: pageNo }) }}>{pageNo}</button></li>)}<li className={`page-item ${rows.length < Number(filters.limit ?? 10) ? 'disabled' : ''}`}><button className="page-link" onClick={() => { const page = (filters.page ?? 1) + 1; setFilters((p) => ({ ...p, page })); void load({ page }) }}>Next</button></li></ul></nav></div>
