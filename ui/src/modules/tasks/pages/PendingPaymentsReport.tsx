...
const PendingPaymentsReport = () => {
  ...
  return (
    <PageContainer title="Pending Payments Report" description="Track unpaid invoices and pending collections.">
      <section className="filter-card mb-3">
        <div className="filter-summary-row">
          <div className="filter-summary-pill"><strong>Total Pending Payments:</strong> {summary.total_pending_tasks}</div>
          <div className="filter-summary-pill"><strong>Total Pending Amount:</strong> {formatCurrency(summary.total_pending_amount)}</div>
        </div>
      ...
      <section className="table-card">
        ...
        <div className="table-wrapper manager-reports-table__wrapper">
          <table className="table table-hover table-bordered align-middle manager-reports-table mb-0">
            ...
            <tbody>{loading ? <tr><td colSpan={7} className="text-center">Loading...</td></tr> : pagedRows.length === 0 ? <tr><td colSpan={7} className="text-center">No pending payments found.</td></tr> : pagedRows.map((row) => (<tr key={row.id}><td>{formatDate(row.due_date || row.created_at)}</td><td>{row.company_name || '—'}</td><td>{row.candidate_name || '—'}</td><td>{formatCurrency(row.pending_amount || row.total_amount)}</td><td><span className="status-pill status-pill--pending">{row.status || 'pending'}</span></td><td>{row.duration}</td><td><NavAction /></td></tr>))}</tbody>
          </table>
        </div>
...
const NavAction = () => <NavLink className="button" to="/tasks/payment-correction">View</NavLink>
