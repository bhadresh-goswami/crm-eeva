...
import {
  ...
  type TechVsTaskDetailRow,
  type TechVsTasksSummaryRow,
} from '../api/tasksApi'
...
const TechVsTasksReport = () => {
  ...
  const [rows, setRows] = useState<TechVsTasksSummaryRow[]>([])
  const [loading, setLoading] = useState(false)
  const [lastUpdated, setLastUpdated] = useState<Date | null>(null)

  const [selected, setSelected] = useState<TechVsTasksSummaryRow | null>(null)
  const [detailRows, setDetailRows] = useState<TechVsTaskDetailRow[]>([])
  const [detailLoading, setDetailLoading] = useState(false)
  const [detailPage, setDetailPage] = useState(1)
  const [sortBy, setSortBy] = useState<SortableKey>('task_id')
  ...
  const openDetails = async (row: TechVsTasksSummaryRow) => {
    setSelected(row)
    setDetailPage(1)
    setSortBy('task_id')
    ...
    const payload = {
      ...
      expert_name: row.technical_expert,
      ...
      <div className="modal fade show d-block" tabIndex={-1} role="dialog" aria-modal="true">
        <div className="modal-dialog modal-xl modal-dialog-scrollable">
          <div className="modal-content">
            <div className="modal-header"><h5 className="modal-title">{selected.technical_expert || 'Technical Expert'} - Task Details</h5><button className="btn-close" onClick={() => setSelected(null)} /></div>
            <div className="modal-body">
              <div className="table-responsive"><table className="table table-hover table-bordered align-middle table-sm"><thead className="table-light"><tr><th>{renderSortHeader('Task ID', 'task_id')}</th><th>{renderSortHeader('Candidate Name', 'candidate_name')}</th><th>{renderSortHeader('Client Company', 'client_company')}</th><th>{renderSortHeader('Task Type', 'task_type')}</th><th>{renderSortHeader('Status', 'status')}</th><th>{renderSortHeader('Task Date', 'task_date')}</th><th>{renderSortHeader('EST Time', 'est_time')}</th><th>{renderSortHeader('Duration', 'duration')}</th><th>{renderSortHeader('Feedback Status', 'feedback_status')}</th><th>{renderSortHeader('Average Score', 'average_score')}</th><th>{renderSortHeader('Assigned By', 'assigned_by')}</th><th>Action/View</th></tr></thead><tbody>{detailLoading ? <tr><td colSpan={12} className="text-center">Loading...</td></tr> : pagedDetailRows.length === 0 ? <tr><td colSpan={12} className="text-center">No tasks found for selected filters.</td></tr> : pagedDetailRows.map((r) => <tr key={r.task_id}><td>{r.task_id}</td><td>{r.candidate_name}</td><td>{r.client_company}</td><td>{r.task_type}</td><td>{r.status}</td><td>{r.task_date}</td><td>{r.est_time}</td><td>{r.duration}</td><td>{r.feedback_status}</td><td>{r.average_score ?? '--'}</td><td>{r.assigned_by}</td><td><button className="btn btn-outline-primary btn-sm"><BsEye/></button></td></tr>)}</tbody></table></div>
