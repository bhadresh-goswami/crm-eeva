...
const TechVsTasksReport = () => {
  ...
  const openDetails = async (row: TechVsTasksSummaryRow) => { setSelected(row); setDetailLoading(true); const payload = { expert_id: row.expert_id, expert_name: row.technical_expert, status: 'completed', from_date: filters.from_date || '', to_date: filters.to_date || '', limit: 1000 }; console.log('Task detail payload:', payload); try { const response = await getTechVsTaskDetails(payload); setDetailRows(Array.isArray(response) ? response : []) } finally { setDetailLoading(false) } }
