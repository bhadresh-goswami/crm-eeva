...
type GroupKey = 'pending' | 'week' | 'month' | 'previous'
...
const groupInfo: Record<GroupKey, { title: string; subtitle: string; empty: string }> = {
  ...
  previous: { title: 'Previous Feedback', subtitle: 'Historical feedback', empty: 'No previous feedback found.' },
}
...
const ExpertTaskReportsPage = () => {
  ...
  const [groups, setGroups] = useState<Record<GroupKey, Row[]>>({ pending: [], week: [], month: [], previous: [] })
  const [totals, setTotals] = useState<Record<GroupKey, number>>({ pending: 0, week: 0, month: 0, previous: 0 })
  const [open, setOpen] = useState<Record<GroupKey, boolean>>({ pending: true, week: true, month: true, previous: true })
  ...
  const dates = useMemo(() => { const now = new Date(); const local = new Date(now.getTime() - now.getTimezoneOffset() * 60000); const iso = (d: Date) => d.toISOString().slice(0, 10); let from = filters.date_from; let to = filters.date_to; if (filters.date_preset === 'today') from = to = iso(local); if (filters.date_preset === 'week') { const d = new Date(local); d.setDate(d.getDate() - ((d.getDay() + 6) % 7)); from = iso(d); to = iso(local) } if (filters.date_preset === 'month') { from = iso(new Date(local.getFullYear(), local.getMonth(), 1)); to = iso(local) } return { from, to } }, [filters])
  const fetchRows = useCallback(async () => {
    ...
    try {
      ...
      const keys: GroupKey[] = ['pending', 'week', 'month', 'previous']
      const results = await Promise.all(keys.map(group => loadTaskForFeedback({ ...common, feedback_group: group, page: group === 'previous' ? page : 1, limit: group === 'previous' ? limit : 500 })))
  ...
  return <PageContainer><ExpertWorkspaceHeader title="Task Feedback" />
    <main className="feedback-workspace">
      ...
      {(Object.keys(groupInfo) as GroupKey[]).map(key => { const info = groupInfo[key]; const totalPages = Math.max(1, Math.ceil(totals.previous / limit)); const numbered = pagesToShow(page, totalPages); return <section key={key} className={`feedback-group group-${key}`}><button className="feedback-group-header" onClick={() => setOpen(p => ({ ...p, [key]: !p[key] }))}>{open[key] ? <BsChevronDown /> : <BsChevronRight />}<span><strong>{info.title}</strong><small>{info.subtitle}</small></span><em>{totals[key]} {key === 'pending' ? 'Pending' : 'Feedback'}</em></button>{open[key] && <><FeedbackTable rows={groups[key]} loading={loading} pending={key === 'pending'} empty={info.empty} onAction={r => action(r, key === 'pending' ? 'ADD' : 'VIEW')} />{key === 'previous' && <footer className="feedback-pagination"><span>Showing {totals.previous ? (page - 1) * limit + 1 : 0}–{Math.min(page * limit, totals.previous)} of {totals.previous} feedback records</span><label>Rows per page <select value={limit} onChange={e => { setLimit(+e.target.value); setPage(1) }}>{[10, 50, 100, 200, 500].map(n => <option key={n}>{n}</option>)}</select></label><nav><button disabled={page === 1} onClick={() => setPage(p => p - 1)}>‹</button>{numbered.map((n, i) => <span key={n}>{i > 0 && n - numbered[i - 1] > 1 && <i>…</i>}<button className={n === page ? 'current' : ''} onClick={() => setPage(n)}>{n}</button></span>)}<button disabled={page === totalPages} onClick={() => setPage(p => p + 1)}>›</button></nav></footer>}</>}</section> })}
