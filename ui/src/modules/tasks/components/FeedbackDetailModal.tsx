...
const FeedbackDetailModal = ({ open, data, onClose }: FeedbackDetailModalProps) => {
  ...
  const apiFields = record.visible_fields
  const configured = getFeedbackConfiguration(taskType, apiFields)
  ...
  let fields: FeedbackFieldConfiguration
  if (Object.keys(configured).length > 0) {
    fields = configured
  } else {
    fields = Object.fromEntries([
      ...Object.keys(legacyLabels).filter((name) => hasValue(record[name])).map((name) => [name, { label: legacyLabels[name], type: 'text' as const, section: ['strengths'].includes(name) ? 'Strengths' : ['recommendations'].includes(name) ? 'Recommendations' : name === 'next_action' ? 'Next Action' : 'Feedback Details' }]),
